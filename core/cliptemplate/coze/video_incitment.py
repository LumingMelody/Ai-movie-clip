...
from core.cliptemplate.coze.transform.coze_video_incitment import trans_video_inciment




def get_video_incitment(title):
    # Init the Coze client through the access_token.
    coze = Coze(auth=TokenAuth(token=coze_api_token), base_url=coze_api_base)

    # Create a workflow instance in Coze, copy the last number from the web link as the workflow's ID.
    workflow_id = '7503745525142192147'
    ...
    result_path=trans_video_inciment(response)

    return result_path


if __name__ == "__main__":
    get_video_incitment("财税")
