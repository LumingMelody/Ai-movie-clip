...
def get_trans_video_clothes_diffrent_scene(input:str):
    ...
    txt_clip1_ori = TextClip(font="微软雅黑.ttf",text=caption_ori[:half_caption_ori], font_size=20, color="black",duration=oriimg_video_clip.duration)
    txt_clip2_ori = TextClip(font="微软雅黑.ttf",text=caption_ori[half_caption_ori:], font_size=20, color="black",duration=oriimg_video_clip.duration)
    ...
    for i, img_url in enumerate(input['img_lists']):
        ...
        txt_clip1 = TextClip('微软雅黑.ttf',caption[:half_caption], font_size=20, color='black', duration=video_clip.duration)
        txt_clip2 = TextClip('微软雅黑.ttf',caption[half_caption:], font_size=20, color='black', duration=video_clip.duration)
    ...
    with open("bgm.mp3", 'wb') as f:
        f.write(response.content)
    bgm = AudioFileClip("bgm.mp3").subclipped(0, final_audio.duration).with_effects([afx.MultiplyVolume(0.2)]) # 调整背景音乐音量
