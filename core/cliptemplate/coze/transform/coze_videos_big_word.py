import requests
from moviepy import AudioFileClip, ImageClip, concatenate_videoclips, CompositeAudioClip,afx
import os
import uuid
...
def trans_video_big_word(data:dict)->str:
  ...
  with open("bgm.mp3", 'wb') as f:
      f.write(response.content)
  bgm = AudioFileClip("bgm_audio.mp3").subclipped(0, final_video.duration).with_effects([afx.MultiplyVolume(0.2)]) # 调整背景音乐音量
